...
SELECT * FROM meal JOIN review ON meal.id = review.meal_id WHERE review.title = 'good' OR review.title = 'very good';
-- Get reservations for a specific meal sorted by created_date
SELECT * FROM meal JOIN reservation ON meal.id = reservation.meal_id WHERE reservation.created_date;
-- Sort all meals by average number of stars in the reviews
SELECT meal.title, AVG(review.stars) AS average_no_of_stars FROM meal JOIN review ON meal.id = review.meal_id
GROUP BY meal.title ORDER BY average_no_of_stars;	 
